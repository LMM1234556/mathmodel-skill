...
2. Re-open the linked official rules and confirm they are still current for the contest year; separately obtain the opening notice, paper format specification, official standard document/template, submission manual, AI notice, and selected problem statement. Add them as distinct source IDs with hashes where downloadable.
3. Update every affected category, unresolved item, and conflict in the snapshot. Official template/cover, typography/paragraphs, pagination/page limits, and appendix/supporting materials must be separate categories; one generic “format checked” fact is invalid. Synchronize the summary to `decision_log.compliance.ruleset`.
...
| `10_appendix.md` | Conditional appendix draft for internal review; include in the official paper only when current rules/prompt allow it |
| `supporting_materials_manifest.md` | Internal file manifest, hashes, requirement anchors, archive name/type/size; never rendered into the paper by default |
...
| `format_audit.md` | Official requirement → observed final format → evidence → pass/fail |
...
First create result cards and `claim_evidence_matrix.md`; then write the body and
references. Create an in-paper appendix only when the current official documents
or selected prompt permit or require it; keep separately uploaded programs/results
in `supporting_materials_manifest.md`, not automatically in the paper. Write the abstract/summary last from locked Claim
...
Copy `templates/shared/format_audit.md` into `paper_workspace/` and fill one row per requirement. For every row record the target-year official value, source ID plus page/anchor, observed artifact value, and result. Inspect at least fonts, heading and paragraph formatting, page geometry, pagination, abstract limit, total-paper limit, cover, anonymity, appendix, PDF properties, filenames, supporting materials, and problem-specific deliverables.
...
- all required sections and problem-specific deliverables exist; appendix content and separately uploaded supporting files follow the current official rule and prompt rather than the internal skeleton;
...
- `paper_workspace/format_audit.md` identifies every unknown or provisional format item and does not invent a total-page, appendix, margin, or supporting-material rule;

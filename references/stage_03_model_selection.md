...
评分前先做不可补偿的可行性筛查。任一候选若不能完成同一题目交付物、所需数据不存在、
关键假设与题面冲突、无法在剩余时间内实现，或没有可执行的验证方式，直接 `reject`；
不得靠“论文展示效果”或模型复杂度的高分抵消这些缺陷。
...
矩阵之外另写一张候选说明卡，明确数据充分性、关键假设、实现难点、结果解释方式和
潜在失败模式。“便于写论文”只能表示数学逻辑与结果可清楚说明，不能成为牺牲正确性、
验证质量或数据适配性的理由。

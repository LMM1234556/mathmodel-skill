...
# mathmodel-skill — 华为杯数学建模工作流 (v7.3)
...
**v7.3 范围**：当前发行版只支持华为杯。其他赛事资料若仍存在于仓库，只是未来扩展的非活动历史资源，不得加载、不得作为当前规则来源，也不属于本版本的质量承诺。
...
本 skill v7.3 以 Codex Skills 为一等入口, 同时保持 harness-agnostic 设计:
...
华为杯当前按 100 小时中文竞赛准备。2026 开赛公告、论文格式、官方 `.doc` 模板、提交手册和 AI 规则已核验；题面、附件、赛中更正与专项交付物仍须从实际题目包和官方通知动态确认。内部 LaTeX 不能作为提交件。反馈深度由 mode 决定。
...
- **stage 0 / 8 / 9**: 读取 `references/rule_verification_protocol.md`，核对 `competitions/huawei/current_rules.md` 与 `official_rules_2026.json` 中的官方链接，维护 `state/rules_snapshot.json`；格式必须分别核对官方模板/封面、字体/段落、分页/页数、附录/支撑材料，不接受笼统的“格式已检查”；依次运行 `audit_ruleset.py --phase kickoff|writing|final`
...
- “切到其他比赛” → 说明 v7.3 仅支持华为杯并停止；不得改写已有项目的 competition
...
- `competitions/huawei/`: 2026 开赛公告、论文格式、官方 `.doc` 模板、提交手册与 AI 规则已于 2026-09-23 核对；题面与赛中更正仍须动态确认。第三方 LaTeX 和本仓库内部模板不得作为官方提交模板，empirical 为 `n=0`
